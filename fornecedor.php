    ...
    if(empty($_POST['nome1']) && (empty($_POST['nome2']))){
        ...
        if($_POST['TipoPessoa'] == "1"){
            ...
            $rSocial = $_POST['razaoSocial'];
                ...
                if(!empty($objColaborador->verificaNomeFornecedor($nome))){
                ...
                $ultimoId = $objColaborador->queryInsertFornecedor($_POST);
                ...
                //identificador identifica a quem pertence o documento cliente(1), colaborador(2), FUNCIONÁRIO(3) 
                    ...
                    if($_POST['TipoPessoa'] == "1"){
                        ...
                        //cadastra documento
                        $docpf = $objColaborador->queryInsertDocPF($dados);
                    ...
                    //CASO O CLIENTE FOR PESSOA JURIDICA 
                       ...
                       //PARA INSERIR OS DOCUMENTOS CNPF, INSC ESTADUAL, INSC MUNICIPAL NA TABELA DOCUMENTOS
                        ...
                        'IMUN'=> $_POST['iMunicipal'],
                         ...
                         $docPJ = $objColaborador->queryInsertDocPJ($dados);
                            ...
                            //cadastra endereço 
                            $insertEnd = $objColaborador->queryInsertEnd($end);
                                  ...
                                  $ctt = array(
                                         ...
                                         'cttStatus'=> $_POST['status']
                                 
                             );
                        //cadastra contato   
                        if($objColaborador->queryInsertCTT($ctt) == 'ok'){
                            
                         $_SESSION['msg'] = "Fornecedor Cadastrado com sucesso.";
                           
                        }else{
                              $_SESSION['msg'] = "Não foi possível cadastrar o fornecedor, tente novamente ou entre em ontato com o desenvolvedor.";
                        }                     
            }        
...
if(isset($_POST['btnCadastrar']) && $_POST['btnCadastrar']=='Alterar'){  
    ...
    if($_GET['tipoFornecedor'] == '1'){       
        
        if($objColaborador->queryUpdateFornecedorPF($user, $nome) == 'ok'){ 
             //echo "o nome foi alterado com sucesso";  
             //echo $cpf;
            if($objColaborador->queryUpdateDocPFFornecedor($user,$cpf)){
                //echo "o cpf foi alterado com sucesso"; 
                
                if($objColaborador->queryUpdateEndFornecedor($user, $dados) == 'ok'){
                
                //echo "Documento Alterado";
                
                if($objColaborador->queryUpdateCTTFornecedor($user, $dados) == 'ok'){
            ...
            if($objColaborador->queryUpdateFornecedorPJ($user, $dados) == 'ok'){   
                
                if($objColaborador->queryUpdateDocPJFornecedor($user, $dados) == 'ok'){
    ...
    if(isset($_GET['acao']) AND $_GET['acao'] == 'excluir'){    
        $user = $objFuncao->base64($_GET['user'], 2);
            if($objColaborador->queryDeleteFornecedor($user) == 'ok'){
                
                if($objColaborador->queryDeleteEnderecoFornecedor($user) == 'ok'){
                            
                    if($objColaborador->queryDeleteContatoFornecedor($user) == 'ok'){
                        
                        if($objColaborador->queryDeleteDocumentoPFFornecedor($user) == 'ok'){
                            
                            $_SESSION['msg'] = "Colaborador Excluido com sucesso.";
                            //header('location: ?pg');
                        }else{
                            echo '<script type="text/javascript">alert("Erro em deletar");</script>';
                        }
                        
                        if($objColaborador->queryDeleteDocumentoPJFornecedor($user) == 'ok'){
...
<head>
	...
	<title>Fornecedor - MVArquitetura</title>
    ...
    ?>
        ...
        <div class="my-3 p-3 bg-white rounded shadow-sm">
            <table cellspacing="4" cellpadding="6">
                <tr>
                    <th>Buscar Fornecedor:</th>
                        </tr>
                        <form name="busca" method="GET" action="exibeFornecedor.php"  class="form-inline my-2 my-lg-0">
                            <tr>  
                                <td>
                                        <input type="text" name="fornecedor" id="search-fornecedor" placeholder="Nome Fornecedor" size="20" required="required" class="form-control"/>
                                        <input type="hidden" name="idFornecedor" id="search-idfornecedor" />
                                        <input type="hidden" name="tipoFornecedor" id="search-tipofornecedor" />
                                        <input type="hidden" name="pg" />
                                        <div id="suggesstion-fornecedor"></div>  
                                </td>
                ...
                <h6 class="border-bottom border-gray pb-2 mb-0"><a href="?pg=0&acao=novo">Novo Fornecedor</a></h6>
...
<div class="container">
 ...
 <?php
 //Exibe o form para cadastrar novo fronecedor
...
//Exibe o form para atualizar usuario
  if(isset($_GET['acao']) AND $_GET['acao'] == 'alterar'){    
    ...
    $userID = $objFuncao->base64($_GET['user'], 2);
    ...
    <div class="media text-muted pt-3"> 
        ...
        <div class="col-md-auto">
            ...
            <form method="POST" action="" class="form-signin">   
                ...
                <h5>Alterar/Incluir Dados do Fornecedor</h5>
                    ...
                    ';
                        ...
                        if($tipo == 1){
                            ...
                            $up = $objColaborador->querySelecionaFornecedorPF($userID, $tipo);
                            ...
                            foreach ($up as $resPF) {}
                                ...
                                echo'   
                                        <label for="nome">Nome:</label><br/>
                                        <input name="nome" type="text" value="'.$resPF->nomeFornecedor.'" class="form-control" /><br/>
                            ...
                            $up = $objColaborador->querySelecionaFornecedorPJ($userID, $tipo);
                            
                                foreach ($up as $resPJ) {}
                                
                                   //mascara para todos os tipos de documentos
                                    $cnpj = $objFuncao->mask($resPJ->cnpj, '##.###.###/####-##');
                                    
                                        echo'
                                        <label for="nome">Nome Fantasia:</label><br/>
                                        <input name="nome" type="text" value="'.$resPJ->nomeFornecedor.'" class="form-control" /><br/>
...
<!--****************************Corte**********************************  -->
        ...
        <div class="my-3 p-3 bg-white rounded shadow-sm">
        <h6 class="border-bottom border-gray pb-2 mb-0">Fornecedores Cadastrados</h6>
        <div class="media text-muted pt-3">
            <table class="table">
                <thead>
                <tr>
...
$quantreg = $objColaborador->querySelectTotalFornecedor();
 $i = $quantreg;
           foreach($objColaborador->querySelectPagFornecedor($inicial, $numreg) as $res){

          echo ' <tr>
                    <td>'.$i--.'</td>
                    <td>'.$res->nomeFornecedor . '</td>
                ';
             if(empty($res->email)){
                
                echo '<td><a href="?pg='.$pg.'&acao=alterar&user='.$objFuncao->base64($res->idFornecedor, 1).'&tipoFornecedor='.$res->tipoFornecedor.'" title="Cadastrar e-mail">Cadastrar e-mail</td>';
            ...
            if($res->dddTel == ('0')){
                ...
                echo '<td><a href="?pg='.$pg.'&acao=alterar&user='.$objFuncao->base64($res->idFornecedor, 1).'&tipoFornecedor='.$res->tipoFornecedor.'" title="Cadastrar Telefone.">Cadastrar Telelefone</td>';
            ...
            if($res->dddCel == ('0')){
                ...
                echo '<td><a href="?pg='.$pg.'&acao=alterar&user='.$objFuncao->base64($res->idFornecedor, 1).'&tipoFornecedor='.$res->tipoFornecedor.'" title="Cadastrar Celular.">Cadastrar Celular</td>';
        ...
        echo'  
                     <td><a href="exibeFornecedor.php?pg='.$pg.'&acao=ver&user='.$objFuncao->base64($res->idFornecedor, 1).'&tipoFornecedor='.$res->tipoFornecedor.'" title="Visualizar esse dado"><img src="images/ico-visualizar.png"  height="16" alt="Visualizar"></td>
                    <td><a href="?pg='.$pg.'&acao=alterar&user='.$objFuncao->base64($res->idFornecedor, 1).'&tipoFornecedor='.$res->tipoFornecedor.'" title="Alterar esse dado"><img src="images/ico-editar.png" width="16" height="16" alt="Editar"></td>
                    <td><a href="?pg='.$pg.'&acao=excluir&user='.$objFuncao->base64($res->idFornecedor, 1).'" title="Excluir esse dado" onclick="return confirm(\'Tem certeza que deseja excluir esse registro?\');"><img src="images/ico-excluir.png" width="16" height="16" alt="Excluir"></td>
                </tr>';
...
if ( $pg > 0) { 		
echo '<tr><td colspan=8 align=center><a href=?pg='.$anterior .' ><b>« anterior</b></td></tr></a>';	
} else { 		
echo "";	
...
if ($pg == ($i_pg-1)) { 			
echo " <tr><td colspan=8 align=center>[$i_pg]</td><t/tr> ";		
} else {			
$i_pg2 = $i_pg-1;			
